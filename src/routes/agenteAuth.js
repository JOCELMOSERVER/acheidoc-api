...
router.post('/recover', async (req, res, next) => {
  try {
    ...
    const otp = gerarOtp();
    await query('DELETE FROM password_reset_tokens WHERE email = $1 AND tipo = $2', [email.toLowerCase(), 'agente']);
    await query(
      'INSERT INTO password_reset_tokens (id, email, tipo, token, expires_at) VALUES ($1, $2, $3, $4, $5)',
      [uuidv4(), email.toLowerCase(), 'agente', otp, otpExpiresAt()]
    );
