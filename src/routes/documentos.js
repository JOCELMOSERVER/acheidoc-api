...
router.post('/', ...requireTipo('utilizador'), async (req, res, next) => {
  try {
    ...
    return res.status(201).json({ documento: created.rows[0] });
  } catch (err) {
    return next(err);
  }
});
...
router.patch('/agente/:id', ...requireTipo('agente'), async (req, res, next) => {
  try {
    ...
    const result = await query('UPDATE documentos SET status = $1 WHERE id = $2 RETURNING id, status', [status, req.params.id]);
    if (!result.rows.length) return res.status(404).json({ erro: 'Documento não encontrado.' });
